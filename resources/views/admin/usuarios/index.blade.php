                    ...
                    <form method="GET" action="{{ route('admin.usuarios.index') }}" class="row g-3">
                        <div class="col-md-6">
                            <input type="text" name="busca" class="form-control"
                                placeholder="Buscar por nome ou email..." value="{{ request('busca') }}">
                        </div>
                        <div class="col-md-3">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($usuario->avaliacoes_count > 0)
                                            <span
                                                class="badge bg-warning text-dark">{{ $usuario->avaliacoes_count }}</span>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex gap-1 justify-content-center">
                                            <a href="{{ route('admin.usuarios.show', $usuario) }}"
                                                class="btn btn-sm btn-outline-primary btn-actions" title="Ver detalhes">
                                                <i class="bi bi-eye"></i>
                                            </a>
                                            ...
                                            <form method="POST" action="{{ route('admin.usuarios.toggle', $usuario) }}"
                                                ...
                                                @csrf
                                                <button type="submit"
                                                    class="btn btn-sm {{ $usuario->ativo ? 'btn-outline-warning' : 'btn-outline-success' }} btn-actions"
                                                    title="{{ $usuario->ativo ? 'Desativar' : 'Ativar' }} usuário">
                                                    <i
                                                        class="bi {{ $usuario->ativo ? 'bi-person-x' : 'bi-person-check' }}"></i>
                                                </button>
                                            </form>
                                        </div>
                                    </td>
            ...
            <div class="card text-center">
                <div class="card-body">
                    ...
                    <h4 class="text-success mt-2">
                        {{ $usuarios->where('created_at', '>=', now()->startOfMonth())->count() }}</h4>
